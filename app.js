...
function setConfig(apiUrl, sharedSecret, autoRefresh) {
  ...
  if (autoRefresh !== undefined) {
    localStorage.setItem(storageKeys.autoRefresh, String(autoRefresh));
  }
}

function hydrateSettings() {
  const config = getConfig();
  if (els.apiUrl) els.apiUrl.value = config.apiUrl;
  if (els.sharedSecret) els.sharedSecret.value = config.sharedSecret;
  if (els.autoRefreshSelect) els.autoRefreshSelect.value = config.autoRefresh;
  updateConnectionChip(Boolean(config.apiUrl), false);
}

function updateConnectionChip(hasUrl, connected) {
  [els.connectionChip, els.connectionChipMirror]
    .filter(Boolean)
    .forEach(chip => {
...
function getVisibleItems() {
  ...
  return sortItems(state.items).filter(item => {
    const matchesSearch =
      !search ||
      [item.name, item.quantity, item.category, item.notes]
        .join(' ')
        .toLowerCase()
        .includes(search);
...
function renderItems() {
  ...
  for (const item of items) {
    ...
    const quantityEl = node.querySelector('.item-quantity');
    if (quantityEl) quantityEl.textContent = `כמות: ${item.quantity || '-'}`;
    ...
    if (dateEl) {
      dateEl.textContent = item.updatedAt ? `עודכן: ${formatDate(item.updatedAt)}` : '';
    }
    ...
    if (deleteBtn) deleteBtn.addEventListener('click', () => deleteItem(item.rowId));
  ...
  if (els.metricTotal) els.metricTotal.textContent = state.items.length;
  if (els.metricDone) els.metricDone.textContent = doneCount;
  if (els.metricLeft) els.metricLeft.textContent = leftCount;
}
...
function renderLists() {
  ...
  state.lists.forEach(list => {
    ...
    if (String(list.id) === String(state.currentListId)) {
  ...
  const currentList = state.lists.find(l => String(l.id) === String(state.currentListId));
  if (els.currentListName) {
...
async function loadLists(showSuccess = false) {
  try {
    const data = await callApi('getLists', {}, 'GET');
    state.lists = data.lists || [];

    if (!state.lists.length) {
      const created = await callApi('createList', { name: 'רשימת קניות' });
      state.remoteVersion = created.version || state.remoteVersion;

      const afterCreate = await callApi('getLists', {}, 'GET');
      state.lists = afterCreate.lists || [];
      state.remoteVersion = afterCreate.version || state.remoteVersion;
    } else {
    ...
    const stillExists = state.lists.some(l => String(l.id) === String(state.currentListId));
    if (!stillExists && state.lists.length) {
      state.currentListId = state.lists[0].id;
    }

    renderLists();

    if (showSuccess) showMessage('הרשימות נטענו בהצלחה.');
    return true;
  } catch (error) {
...
async function reloadAllData({ silent = false } = {}) {
  const ok = await loadLists(false);
  if (!ok) return;
  await loadItems(false, { silent });
}
...
async function addItem(event) {
  ...
  if (!state.currentListId) {
    showMessage('אין רשימה פעילה כרגע.', true);
    return;
  }
  ...
  try {
    ...
    const data = await callApi('add', payload);
    state.remoteVersion = data.version || state.remoteVersion;
...
function setAutoRefresh(seconds) {
  ...
  if (!sec || sec <= 0) {
    setSyncChip('בדיקת שינוי כבויה', 'disconnected');
    return;
  }
...
function switchTab(tab) {
  ...
  els.navBtns.forEach(btn => {
    btn.classList.toggle('active', btn.dataset.targetTab === tab);
  });
...
async function switchList(listId) {
  state.currentListId = String(listId);
  renderLists();
  await loadItems();
}
...
function bindEvents() {
  ...
  els.testConnectionBtn?.addEventListener('click', async () => {
    ...
    setAutoRefresh(els.autoRefreshSelect.value);
    await reloadAllData();
    updateConnectionChip(Boolean(els.apiUrl.value.trim()), true);
  });
  ...
  els.openAddDialogBtn?.addEventListener('click', () => els.addDialog?.showModal());
  els.openAddDialogBtnTab?.addEventListener('click', () => els.addDialog?.showModal());
  els.closeAddDialogBtn?.addEventListener('click', () => els.addDialog?.close());
  ...
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) checkVersionAndSync();
  });
  ...
  els.searchProductBtn?.addEventListener('click', async () => {
    ...
    if (data) {
      const priceInput = els.addItemForm.querySelector('[name="price"]');
      const imageInput = els.addItemForm.querySelector('[name="image"]');

      if (priceInput) priceInput.value = data.price || '';
      if (imageInput) imageInput.value = data.image || '';

      showMessage('נתונים נטענו.');
    } else {
      showMessage('לא נמצאו נתונים.', true);
    }
  });

  els.sidemenu?.addEventListener('click', e => {
    if (e.target === els.sidemenu) closeSidemenu();
  });
}
